...
// rendering. The UNITY_* macros below are required so the glow renders in
// BOTH eyes; without them it appears in one eye only.
//
// Mesh
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes IN)
            {
                ...
                // initialize the eye index output. Must be the first thing
                // that happens, before any position math, because the
                // matrices and _WorldSpaceCameraPos below resolve per-eye
                // based on it.
